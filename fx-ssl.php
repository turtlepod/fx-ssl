...
function fx_ssl_admin_notice() {
	...
	<?php if( 'fail' === get_transient( 'fx_ssl_notice' ) ) : ?>

		<div class="updated notice is-dismissible">
			<?php echo wpautop( sprintf( _x( 'You need to enable (and enforce) <a href="%s" target="_blank">WordPress administration over SSL</a> and use HTTPS URL in "WordPress Address (URL)" and "Site Address (URL)" in <a href="%s">General Settings</a> to use f(x) SSL plugin.', 'activation notice', 'fx-ssl' ), 'https://codex.wordpress.org/Administration_Over_SSL', admin_url( 'options-general.php' ) ) ); ?>
		</div>
		<?php  ?>

	<?php elseif ( 'success' === get_transient( 'fx_ssl_notice' ) ) : ?>

		<div class="updated notice is-dismissible">
			<?php echo wpautop( 'fx_ssl_notice_success', sprintf( _x( 'Navigate to <a href="%s">General Settings</a> to activate SSL (HTTPS) in your site.', 'activation notice', 'fx-ssl' ), admin_url( 'options-general.php' ) ) ); ?>
		</div>

	<?php elseif ( 'active' === get_transient( 'fx_ssl_notice' ) ) : ?>

		<div class="updated notice is-dismissible">
			<?php echo wpautop( _x( 'Your site SSL (HTTPS) is active.', 'activation notice', 'fx-ssl' ) );?>
		</div>
